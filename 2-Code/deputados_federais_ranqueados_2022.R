...
# Carregando os dados ----------------------------------------------------------
path <- "F:/Public/Documents/repositorioTSE/data/output/JoinFinal/votacao_secao_coli_cand_2022_DEPUTADO FEDERAL_CEPESPv202410.parquet"
...
# Carregando os dados 
deputados_federais_2022 <- open_dataset(path) |>
...
deputados_federais_2022$QTDE_VOTO_NOMINAL <- 0
deputados_federais_2022$QTDE_VOTO_LEGENDA <- 0
deputados_federais_2022$QTDE_VOTO_LEGENDA <- ifelse(deputados_federais_2022$NOME_CANDIDATO == "VOTO LEGENDA", 
                                                    deputados_federais_2022$QTDE_VOTOS, 0)

# Atribuíndo votos nominais 
deputados_federais_2022$QTDE_VOTO_NOMINAL <- ifelse(
  # Checando se há NAs para NOME_CANDIDATO
  !is.na(deputados_federais_2022$NOME_CANDIDATO) & 
    # Atribuíndo zero votos nominais a votos na legenda
    deputados_federais_2022$NOME_CANDIDATO != "VOTO LEGENDA" &
    # Atribuíndo zero votos nominais a votos brancos
    deputados_federais_2022$NOME_CANDIDATO != "VOTO BRANCO" &
    # Atribuíndo zero votos nominais a votos nulos 
    deputados_federais_2022$NOME_CANDIDATO != "VOTO NULO",
  deputados_federais_2022$QTDE_VOTOS, 
  0
)

# Identificando coligações e partidos isolados
deputados_federais_2022 <- deputados_federais_2022 |>
  # Variável categórica que identifica nome da legenda
  mutate(IDENTIFICADOR_LEGENDA = ifelse(TIPO_LEGENDA == "FEDERAÇÃO", 
                                        COMPOSICAO_LEGENDA, 
         ...
         COLIGACAO = ifelse(TIPO_LEGENDA == "FEDERAÇÃO", 1, 
                            ifelse(TIPO_LEGENDA == "PARTIDO ISOLADO", 0, NA)),
         # Criando identificador numérico único para legendas
         ID_LEGENDA = ifelse(TIPO_LEGENDA == "FEDERAÇÃO", 
                             sprintf("%03d", as.numeric(factor(COMPOSICAO_LEGENDA))), 
                             ifelse(TIPO_LEGENDA == "PARTIDO ISOLADO", 
...
# Soma votos na legenda: federações
votos_legenda <- deputados_federais_2022 |> 
  group_by(ID_LEGENDA,
           SIGLA_UE) |>
  summarise(TOTAL_VOTOS_LEGENDA = sum(as.numeric(QTDE_VOTO_LEGENDA), na.rm = TRUE))

# Soma total de votos preferenciais: federações e partidos isolados
votos_nominais <- deputados_federais_2022 |> 
  group_by(SIGLA_UE, 
           ID_CEPESP,
           NOME_CANDIDATO,
           NOME_PARTIDO) |>
...
votos_partido <- deputados_federais_2022 |>
...
votos_legenda_partidos <- deputados_federais_2022 |>
...
total_votos_preferenciais <- deputados_federais_2022 |>
  group_by(ID_LEGENDA) |>
...
total_votos_candidato <- deputados_federais_2022 |>
  group_by(ID_CEPESP,
           SIGLA_UE) |>
...
candidatos_individuais <- deputados_federais_2022 |>
  distinct(SIGLA_UE,
           ID_CEPESP,
           .keep_all = TRUE) |>
  filter(!NOME_CANDIDATO %in% c("VOTO LEGENDA",
...
listas_2022_ranqueadas <- candidatos_individuais |>
  group_by(UF, 
           ID_LEGENDA) |>
  arrange(UF, 
          ID_LEGENDA, 
...
listas_partidos_2022_ranqueados <- listas_2022_ranqueadas |> 
  group_by(UF, 
           NOME_PARTIDO) |>
  arrange(UF, 
          NOME_PARTIDO, 
...
resultados <- listas_partidos_2022_ranqueados |>
  group_by(UF, 
           ID_LEGENDA) |>
...
final_final_2022 <- final_lista_partidos |>
  group_by(SIGLA_UE,
           NOME_CANDIDATO) |>
...
final_final_2022 <- final_final_2022 |>
  group_by(UF,
...
final_quebras_2022 <- final_final_2022 |>
  group_by(UF,
           NOME_PARTIDO) |>
  mutate(
...
final_copartisans_2022 <- final_quebras_2022 |>
  group_by(UF, 
           IDENTIFICADOR_LEGENDA) |>
...
final_copartisans_2022 <- final_copartisans_2022 |>
  group_by(UF,
           NOME_PARTIDO) |>
...
# Removendo candidaturas não deferidas
final_copartisans_2022 <- final_copartisans_2022[!(final_copartisans_2022$DES_SITUACAO_CANDIDATURA %in% c("RENÚNCIA", 
                                                                                                          "INDEFERIDO",
...
# Salvando o dado ---------------------------------------------------------
saveRDS(final_copartisans_2022, file = "deputados_federais_ranqueados_2022")
write.csv(final_copartisans_2022, "deputados_federais_ranqueados_2022.csv")
