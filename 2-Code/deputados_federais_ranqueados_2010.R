...
# Carregando os dados ----------------------------------------------------------
path <- "F:/Public/Documents/repositorioTSE/data/output/JoinFinal/votacao_secao_coli_cand_2010_DEPUTADO FEDERAL_CEPESPv202410.parquet"
...
# Carregando os dados 
deputados_federais_2010 <- open_dataset(path) |>
...
deputados_federais_2010$QTDE_VOTO_NOMINAL <- 0
deputados_federais_2010$QTDE_VOTO_LEGENDA <- 0
deputados_federais_2010$QTDE_VOTO_LEGENDA <- ifelse(deputados_federais_2010$NOME_CANDIDATO == "VOTO LEGENDA", 
                                                    deputados_federais_2010$QTDE_VOTOS, 0)

# Atribuíndo votos nominais 
deputados_federais_2010$QTDE_VOTO_NOMINAL <- ifelse(
  # Checando se há NAs para NOME_CANDIDATO
  !is.na(deputados_federais_2010$NOME_CANDIDATO) & 
    # Atribuíndo zero votos nominais a votos na legenda
    deputados_federais_2010$NOME_CANDIDATO != "VOTO LEGENDA" &
    # Atribuíndo zero votos nominais a votos brancos
    deputados_federais_2010$NOME_CANDIDATO != "VOTO BRANCO" &
    # Atribuíndo zero votos nominais a votos nulos 
    deputados_federais_2010$NOME_CANDIDATO != "VOTO NULO",
  deputados_federais_2010$QTDE_VOTOS, 
  0
)

# Identificando coligações e partidos isolados
deputados_federais_2010 <- deputados_federais_2010 |>
...
votos_legenda <- deputados_federais_2010 |> 
  group_by(ID_LEGENDA,
           SIGLA_UE) |>
...
votos_nominais <- deputados_federais_2010 |> 
  group_by(SIGLA_UE, 
           ID_CEPESP,
           NOME_CANDIDATO,
           NOME_PARTIDO) |>
...
votos_partido <- deputados_federais_2010 |>
...
votos_legenda_partidos <- deputados_federais_2010 |>
...
total_votos_preferenciais <- deputados_federais_2010 |>
  group_by(ID_LEGENDA) |>
...
total_votos_candidato <- deputados_federais_2010 |>
  group_by(ID_CEPESP,
           SIGLA_UE) |>
...
candidatos_individuais <- deputados_federais_2010 |>
  distinct(SIGLA_UE,
           ID_CEPESP,
           .keep_all = TRUE) |>
  filter(!NOME_CANDIDATO %in% c("VOTO LEGENDA",
...
listas_2010_ranqueadas <- candidatos_individuais |>
  group_by(UF, 
           ID_LEGENDA) |>
  arrange(UF, 
          ID_LEGENDA, 
...
listas_partidos_2010_ranqueados <- listas_2010_ranqueadas |> 
  group_by(UF, 
           NOME_PARTIDO) |>
  arrange(UF, 
          NOME_PARTIDO, 
...
resultados <- listas_partidos_2010_ranqueados |>
  group_by(UF, 
           ID_LEGENDA) |>
...
final_final_2010 <- final_lista_partidos |>
  group_by(SIGLA_UE,
           NOME_CANDIDATO) |>
...
final_final_2010 <- final_final_2010 |>
  group_by(UF,
...
final_quebras_2010 <- final_final_2010 |>
  group_by(UF,
           NOME_PARTIDO) |>
  mutate(
...
final_copartisans_2010 <- final_quebras_2010 |>
  group_by(UF, 
           IDENTIFICADOR_LEGENDA) |>
...
final_copartisans_2010 <- final_copartisans_2010 |>
  group_by(UF,
           NOME_PARTIDO) |>
...
# Salvando o dado ---------------------------------------------------------
saveRDS(final_copartisans_2010, file = "deputados_federais_ranqueados_2010")
write.csv(final_copartisans_2010, "deputados_federais_ranqueados_2010.csv")
