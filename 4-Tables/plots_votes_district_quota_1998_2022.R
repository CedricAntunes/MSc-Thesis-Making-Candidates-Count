...
tabela_final <- tabela_final |>
  pivot_wider(names_from = ANO_ELEICAO, 
              values_from = count) |>
  rename("District Quota" = Quotient)
...
# Reformatando a tabela em formato long
tabela_long <- tabela_final |>
  pivot_longer(cols = -`District Quota`, 
               names_to = "ANO_ELEICAO", 
               values_to = "NUM_CANDIDATES")

# Convertend ANO_ELEICAO para numérico
tabela_long$ANO_ELEICAO <- as.numeric(tabela_long$ANO_ELEICAO)

# Dropando thresholds de candidatos fortes
tabela_long_filtered <- tabela_long |>
  filter(`District Quota` != "200%" & `District Quota` != "150%")

# Série temporal
ggplot(tabela_long_filtered, aes(x = ANO_ELEICAO, 
                                 y = NUM_CANDIDATES, 
                                 color = `District Quota`, 
                                 group = `District Quota`)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = seq(1998, 2022, 4)) +  
  labs(title = "Candidates' Vote Share as Ratio of the District Quota (1998-2022)",
       subtitle = "Votes Shares in Preferential Votes",
       x = "Electoral Year",
...
# Barplot
ggplot(tabela_long_filtered, aes(x = ANO_ELEICAO, y = NUM_CANDIDATES, fill = `District Quota`)) +
...
# Stacked barplot
ggplot(tabela_long_filtered, aes(x = as.factor(ANO_ELEICAO), y = NUM_CANDIDATES, fill = `District Quota`)) +
